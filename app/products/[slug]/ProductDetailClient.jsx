  ...
  return (
    <div className="min-h-screen bg-white pb-20 pt-10">
      <div className="container mx-auto px-6 lg:px-20">
        ...
        {/* Breadcrumbs */}
        <nav className="flex items-center flex-nowrap whitespace-nowrap gap-1 md:gap-2 text-[10px] md:text-[13px] text-zinc-400 font-medium mb-10 overflow-x-auto no-scrollbar">
          ...
          <ChevronRight size={12} className="flex-shrink-0 md:w-3.5 md:h-3.5" />
          <span className="text-zinc-300 capitalize truncate max-w-[150px] md:max-w-none">{product?.title}</span>
        </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <div className="flex flex-col-reverse md:flex-row gap-6">
            ...
            <div className="flex md:flex-col gap-4 overflow-x-auto md:overflow-visible no-scrollbar">
              {(product.images || [product.thumbnail]).map((img, i) => (
                ...
                >
                  <Image src={img} alt={`${product.title} view ${i+1}`} fill sizes="96px" quality={60} className="object-cover" />
                </div>
              ))}
            </div>
            {/* Main Image */}
          ...
          <div className="flex flex-col">
            ...
            <h1 className="text-3xl md:text-5xl font-medium text-black leading-[1.1] mb-6">
              {product.title}
            </h1>
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
              <div className="flex items-center gap-4">
                {product.discountPercentage > 0 && (
                  <span className="text-xl font-medium text-zinc-300 line-through tracking-tighter">
                    ${Math.round(product.price * (1 + product.discountPercentage / 100))}
                  </span>
                )}
                <span className="text-3xl font-medium text-black tracking-tighter">${Number(product.price).toFixed(2)}</span>
              </div>
              
              <div className="flex items-center gap-6">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-zinc-400">{sold} Sold</span>
            ...
            <div className="space-y-4 mb-12">
              <h4 className="text-lg font-medium text-black uppercase tracking-tight">Description:</h4>
              <p className="text-[16px] text-zinc-500 leading-relaxed font-medium">
                ...
                <button 
                  ...
                  className="text-black font-medium cursor-pointer ml-2 hover:underline focus:outline-none"
                >
            ...
            <div className="flex items-center gap-4 pt-6 border-t border-gray-50">
              ...
              <button 
                ...
                id="add-to-cart-btn"
                className="flex-[2] md:flex-1 h-16 bg-black text-white flex items-center justify-center gap-3 font-medium uppercase text-[10px] md:text-xs tracking-[0.2em] md:tracking-[0.3em] hover:bg-neutral-800 transition-all hover:scale-[1.02] active:scale-95 shadow-2xl rounded-full btn-animate min-w-[120px]"
              >
              ...
              <button 
                ...
                className={`w-16 h-16 border border-gray-100 flex items-center justify-center transition-all cursor-pointer hover:shadow-lg active:scale-90 rounded-full
                  ${isInWishlist(product.id) ? 'bg-red-50 border-red-100 text-red-500' : 'bg-white text-zinc-400 hover:text-black'}
                `}
              >
