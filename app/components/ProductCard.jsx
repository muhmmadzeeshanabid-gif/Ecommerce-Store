  ...
  return (
    <div className="group flex flex-col gap-4 relative">
      ...
      <div className="relative aspect-[4/5] overflow-hidden bg-gray-100 rounded-[20px]">
        ...
        <button 
          ...
          className={`absolute top-4 right-4 z-10 p-2.5 rounded-full shadow-lg transition-all duration-300 backdrop-blur-sm cursor-pointer
            ${isInWishlist(product.id) ? 'bg-red-500 text-white scale-110' : 'bg-white/80 text-zinc-400 hover:text-red-500 hover:bg-white'}
          `}
        >
      ...
      <div className="flex flex-col gap-1 px-1">
        <h3 className="text-[16px] md:text-[18px] font-medium text-black leading-tight tracking-tight line-clamp-2 min-h-[48px]">
           {product.title}
        </h3>
        <p className="text-[14px] md:text-[16px] font-medium text-black mt-1">
           ${product.price}
        </p>
        
        {/* Unique Rating & Sold Stats based on Product Hashing */}
        <div className="flex items-center gap-1.5 mt-1 text-[12px] md:text-[13px] text-zinc-400 font-medium">
