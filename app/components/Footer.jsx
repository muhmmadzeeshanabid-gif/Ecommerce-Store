        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-12">
          ...
          <div className="sm:col-span-2 lg:col-span-1 space-y-8">
            ...
            <div className="flex gap-3">
              {/* Facebook */}
              <a href="#" className="w-10 h-10 rounded-full border border-gray-200 flex items-center justify-center text-neutral-400 hover:bg-black hover:text-white hover:border-black transition-all duration-300">
          ...
          <div className="space-y-6">
            <p className="text-sm font-medium uppercase tracking-widest text-black">Shop</p>
            <ul className="space-y-4">
              {categories.map((cat) => (
                <li key={cat.slug}>
                  <Link href={`/categories/${cat.slug}`} className="text-sm text-neutral-500 hover:text-black transition-colors">
                    {cat.name}
                  </Link>
                </li>
              ))}
          ...
          {/* Column 3: Help */}
          <div className="space-y-6">
            <p className="text-sm font-medium uppercase tracking-widest text-black">Help</p>
            <ul className="space-y-4">
              {['Customer Support', 'Delivery Details', 'Returns & Refunds', 'Terms', 'Privacy'].map((l) => (
                <li key={l}>
                  <Link href="#" className="text-sm text-neutral-500 hover:text-black transition-colors">{l}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          {/* Column 4: Contact */}
          <div className="space-y-6">
            <p className="text-sm font-medium uppercase tracking-widest text-black">Contact</p>
            <div className="space-y-4">
              <a href="mailto:support@zara.st" className="flex items-center gap-3 text-sm text-neutral-500 hover:text-black transition-colors">
                <Mail size={14} />
                support@zara.st
              </a>
              <a href="tel:+12025550192" className="flex items-center gap-3 text-sm text-neutral-500 hover:text-black transition-colors">
                <Phone size={14} />
                +1 202 555 0192
              </a>
          ...
          <div className="space-y-6">
            <p className="text-sm font-medium uppercase tracking-widest text-black">Newsletter</p>
            {subscribed ? (
              <div className="pt-2 animate-in fade-in slide-in-from-left-4 duration-700">
                <p className="text-[12px] font-medium uppercase tracking-[0.3em] text-black">
                  THANK YOU.
                </p>
                ...
                <form onSubmit={handleSubscribe} className="space-y-3">
                  <div className="flex items-center gap-0 bg-gray-50 border border-gray-200 p-1.5 rounded-full hover:border-black transition-all focus-within:border-black">
                    <input
                      ...
                      className="min-w-0 flex-1 bg-transparent px-4 py-2 text-xs text-black placeholder:text-neutral-300 outline-none"
                    />
      ...
      <div className="border-t border-gray-100">
        <div className="container mx-auto px-6 lg:px-20 py-8 flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="h-8 px-4 bg-gray-50 border border-gray-100 rounded-md flex items-center">
              <span className="text-[9px] font-bold text-neutral-500">AMEX</span>
            </div>
          </div>
        </div>
      </div>
