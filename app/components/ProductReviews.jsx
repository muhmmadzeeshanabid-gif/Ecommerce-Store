...
const ProductReviews = ({ reviews = [], productDescription = "", productRating = 4.5 }) => {
  ...
  return (
    <div className="mt-20 py-16 bg-white font-inter">
      <div className="container mx-auto px-6 lg:px-20">
        <h2 className="text-2xl font-bold text-neutral-900 mb-8 font-inter">Product Reviews</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
            <div className="md:col-span-3 flex items-center gap-6">
              <div className="w-20 h-20 md:w-24 md:h-24 rounded-full border-2 border-orange-400 flex flex-col items-center justify-center bg-white shadow-sm">
                <span className="text-2xl md:text-3xl font-bold text-neutral-900">{productRating}</span>
              </div>
              <div className="flex flex-col">
                <div className="flex gap-0.5 mb-1">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} size={18} className={`${i < Math.round(productRating) ? "text-orange-400 fill-orange-400" : "text-neutral-200"}`} />
                  ))}
                </div>
                <p className="text-[12px] text-neutral-400 font-bold uppercase tracking-tight">from {reviews.length} reviews</p>
                <button 
            ...
            <div className="md:col-span-9 space-y-3">
              {[5, 4, 3, 2, 1].map((star) => (
                <div key={star} className="flex items-center gap-4">
                  <div className="flex items-center gap-1 w-10 text-[13px] font-medium text-neutral-700">
                  ...
                  <div className="w-12 text-right text-[13px] font-medium text-neutral-400">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          <aside className="lg:col-span-3 border border-dashed border-gray-200 rounded-xl p-6 h-fit lg:sticky lg:top-24 shadow-sm bg-white/50">
             <h3 className="text-base font-bold text-neutral-900 mb-8 border-b border-gray-100 pb-4">Reviews Filter</h3>
             <div className="space-y-10">
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-[14px] font-bold text-neutral-800">Rating</span>
                  ...
                  <div className="space-y-3">
                    {[5, 4, 3, 2, 1].map((s) => (
                      ...
                      >
                        ...
                        <div className={`flex items-center gap-1 text-[13px] font-medium ${selectedRatings.includes(s) ? 'text-neutral-900' : 'text-neutral-500 group-hover:text-neutral-900'}`}>
                          <Star size={12} className="text-orange-400 fill-orange-400" /> {s}
                        </div>
                        <span className="ml-auto text-[11px] text-neutral-300 font-bold">{reviews.filter(r => r.rating === s).length}</span>
                      </button>
                    ))}
                  </div>
                </div>
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-[14px] font-bold text-neutral-800 uppercase tracking-tighter">Review Topics</span>
                    <ChevronUp size={16} className="text-neutral-400" />
                  </div>
                  <div className="space-y-3">
                    {['Product Quality', 'Seller Services', 'Product Price', 'Shipment', 'Match with Description'].map((topic) => (
                      <label key={topic} className="flex items-center gap-3 cursor-pointer group">
                        ...
                        <span className="text-[12px] font-bold text-neutral-500 group-hover:text-neutral-900 uppercase tracking-tight">{topic}</span>
                      </label>
                    ))}
                  </div>
                </div>
          ...
          <main className="lg:col-span-9">
            <h3 className="text-base font-bold text-neutral-900 mb-6 uppercase tracking-tighter">Review Lists</h3>
            <div className="flex flex-row overflow-x-auto no-scrollbar gap-3 mb-12 pb-2">
              {[
                ...
                <button 
                  ...
                  ${activeFilter === chip.label ? 'bg-gray-800 border-gray-900 text-white shadow-xl' : 'bg-white border-gray-100 text-neutral-500 hover:border-gray-800'}`}
                >
                  {chip.icon}
              ...
              {filteredReviews.length > 0 ? filteredReviews.map((rev) => (
                <div key={rev.id} className="pb-10 border-b border-dashed border-gray-100 last:border-0 relative group">
                  <div className="flex gap-0.5 mb-3">
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} size={16} className={`${i < rev.rating ? "text-orange-400 fill-orange-400" : "text-neutral-100"}`} />
                    ))}
                  </div>
                  <h4 className="text-[16px] font-bold text-neutral-900 mb-3 leading-tight tracking-tight uppercase">This is amazing product I have.</h4>
                  <p className="text-[14px] text-neutral-500 font-normal leading-relaxed mb-6 max-w-4xl transition-colors group-hover:text-neutral-800">{rev.comment}</p>
                  
                  <div className="flex items-center justify-between mt-auto">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full border border-gray-100 overflow-hidden bg-gray-50 ring-2 ring-white">
                        <Image src={`https://i.pravatar.cc/150?u=${rev.reviewerName}`} width={36} height={36} quality={60} className="object-cover rounded-full" alt={rev.reviewerName} />
                      </div>
                      <div className="flex flex-col">
                         <span className="text-[13px] font-bold text-neutral-800">{rev.reviewerName}</span>
                    ...
                    <div className="flex items-center gap-3 relative">
                      ...
                      <button 
                        ...
                        className={`flex items-center gap-2 px-4 py-2 border rounded-full transition-all cursor-pointer ${reactions[rev.id] === 'like' ? 'bg-black border-black text-white shadow-lg' : 'bg-white border-gray-100 text-neutral-400 hover:border-black'}`}
                      >
                      ...
                      <button 
                        ...
                        className={`flex items-center justify-center w-11 h-11 border rounded-full transition-all cursor-pointer ${reactions[rev.id] === 'dislike' ? 'bg-red-500 border-red-500 text-white shadow-lg' : 'bg-white border-gray-100 text-neutral-400 hover:border-red-500'}`}
                      >
                ...
                <div className="py-20 text-center border-2 border-dashed border-gray-100 rounded-[32px]">
                   <p className="text-neutral-400 font-bold italic uppercase tracking-widest text-sm">No reviews matching your filters</p>
                </div>
              )}
            </div>
          </main>
